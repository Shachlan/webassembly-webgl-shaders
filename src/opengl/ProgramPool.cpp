...
static string blend_fragment =
    ...
    "\n"
    "void main() {\n"
    "    vec4 color1 = number_of_textures > 0 ? texture2D(tex1, vTexCoord) : vec4(0);\n"
    "    vec4 color2 = number_of_textures > 1 ? texture2D(tex2, vTexCoord) : vec4(0);\n"
    "    vec4 color3 = number_of_textures > 2 ? texture2D(tex3, vTexCoord) : vec4(0);\n"
    "    vec4 color4 = number_of_textures > 3 ? texture2D(tex4, vTexCoord) : vec4(0);\n"
    "    vec4 color5 = number_of_textures > 4 ? texture2D(tex5, vTexCoord) : vec4(0);\n"
    "    vec4 color6 = number_of_textures > 5 ? texture2D(tex6, vTexCoord) : vec4(0);\n"
    "    vec4 color7 = number_of_textures > 6 ? texture2D(tex7, vTexCoord) : vec4(0);\n"
    "    vec4 color8 = number_of_textures > 7 ? texture2D(tex8, vTexCoord) : vec4(0);\n"
    "    gl_FragColor = color1 + color2 + color3 + color4 + color5+color6 + color7 + color8;\n"
    "}";
